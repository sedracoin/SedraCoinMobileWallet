...
class WalletAuthNotifier extends StateNotifier<WalletAuth> {
  ...
  Future<String> _getSeed() async {
    ...
    return SedraUtil.decryptHex(secret, sessionKey);
  }
  ...
  Future<Uint8List> sign(
    ...
    final signature =
        await SedraUtil.computeSignDataSchnorr(data, keyPair.privateKey);
    return signature;
  }
  ...
  Future<void> unlockWithPassword(String password) async {
    ...
    final sessionKey = await walletVault.getSessionKey();
    final encryptedSecret = SedraUtil.encryptHex(seed, sessionKey);
  ...
  HdAddressGenerator addressGenerator(SedraNetwork network) {
