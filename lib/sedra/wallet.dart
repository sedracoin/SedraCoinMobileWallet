...
const kSedraDerivationPath = "m/44'/111111'/0'";
...
String convertIfXpub(String hdPubKey) {
  if (hdPubKey.startsWith('xpub')) {
    try {
      ...
      bip32.network = networkTypeForNetwork(SedraNetwork.mainnet);
...
String convertHdPublicKey(String hdPubKey, SedraNetwork toNetwork) {
...
AddressPrefix addressPrefixForNetwork(SedraNetwork network) {
  switch (network) {
...
abstract class HdWallet implements HdWalletView {
  ...
  static String hdPublicKeyFromSeed(
    ...
    final child = bip32.derivePath(kSedraDerivationPath);
...
class HdWalletEcdsa extends HdWallet {
  ...
  HdWalletEcdsa(Uint8List seed) : super._() {
    _bip32 = BIP32.fromSeed(seed).derivePath(kSedraDerivationPath);
  }
