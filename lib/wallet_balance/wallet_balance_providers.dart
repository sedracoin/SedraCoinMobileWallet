...
final sedraPriceProvider = Provider.autoDispose((ref) {
  return ref.watch(coingeckoSedraPriceProvider);
});
...
final formatedTotalFiatProvider = Provider.autoDispose((ref) {
  // Return an empty string in all cases
  return '';
});

final formatedSedraPriceProvider = Provider.autoDispose((ref) {
  // Return an empty string in all cases
  return '';
});
...
final fiatValueForAddressProvider =
  ...
  final price = ref.watch(sedraPriceProvider);
  final balance = ref.watch(balanceForAddressProvider(address));
...
final sedraFormatterProvider = Provider((ref) {
  final format = NumberFormat.currency(name: '', symbol: 'SDR');
  final formatter = CurrencyFormatter(
    groupSeparator: format.symbols.GROUP_SEP,
    decimalSeparator: format.symbols.DECIMAL_SEP,
    maxDecimalDigits: TokenInfo.sedra.decimals,
    maxAmount: kMaxSedra,
  );
...
final fiatFormatterProvider = Provider.autoDispose((ref) {
  final price = ref.watch(sedraPriceProvider);
  final currency = ref.watch(currencyProvider);
  final maxAmount = price.price * kMaxSedra;
